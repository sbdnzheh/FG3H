...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "L_Y_Y_Y":
      try:
...
@Client.on_message(filters.command(["باندا","المطور باندا ","المطور باندا "], ""))
async def sultan(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="A_A_5n")
     name = user.first_name
     username = user.username 
     bio = user.bio
     user_id = user.id
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["L_Y_Y_Y"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["L_Y_Y_Y"]:
   ...
   if message.from_user.username == "L_Y_Y_Y":
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝙾𝚆𝙽𝙴𝚁 ➧ @L_Y_Y_Y  💎 .**")
